...
app.all('/query', function (req, res, next) {
  ...
  for (var queryId = 0; queryId < req.body.targets.length && !error; queryId++) {
    ...
    if (queryArgs.err != null) {
      ...
      // Run the query
      queryArgs.pipeline = manipulateQuery(queryArgs.pipeline);
...
function isMatchAggregation(aggregation) {
  return aggregation.hasOwnProperty('$match');
}

function isNestedAggregation(aggregation) {
  return aggregation.includes('$');
}

function extractMultipleValues(aggregation) {
...
function manipulateQuery(query) {
  ...
  parsedQuery.forEach(aggregation => {
    if (isMatchAggregation(aggregation)) {
      ...
      Object.entries(matchAggregation).forEach(([key, value]) => {
        const stringifiedAggregation = JSON.stringify(value);

        if (!isNestedAggregation(stringifiedAggregation)) {
